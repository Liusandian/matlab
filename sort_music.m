...
Source_N=1;                          % 信号数
Degrad=pi/180; 
Fre_Jiange=64000/1024;%64000/512; %16000/512;                  %频率间隔,fs=64Khz
c=340;     %c=343;                            %声速
...
Start_Fre_Point=10;                     %起始频率点，512个点傅立叶变换，起始频率约为100HZ
End_Fre_Point=13;                      %结束频率点，512个点傅立叶变换。结束频率约为3750HZ
Fre_Step=1;                          %步长
...
channel44=input2(:,5);
% 
%对获取的每通道数据进行能量大小排序，sort();%实际操作不可行，峰值在0度左右
channel1=sort(channel11);
channel2=sort(channel22);
channel3=sort(channel33);
channel4=sort(channel44);
%
figure(31);
plot(channel3);
% 
% figure(41);
i=0;
Start_Frame_Number=600;%977;%710;%1152;%42;                   %起始帧  (500-600是合理区间)
End_Frame_Number=Frame_Num-1;%3906;%1210;%1502;%75; 600~780
for FrameIndx=Start_Frame_Number:End_Frame_Number     %帧序数
i=i+1;
...
channel1_fftData(((i-1)*Fft_Num+1):i*Fft_Num)=fft(Buf_channel1,Fft_Num );              % 对帧数据进行傅立叶变换，获得变换后的数据fftData    
...
pinlv=(FreIndx-1)*Fre_Jiange;
Y1=X(1:4,((FrameIndx-1)*Fft_Num+FreIndx));                          %选择对应频率点
Y2=X(1:4,(FrameIndx*Fft_Num+FreIndx));
...
Vn=U(:,Source_N+1:Array_N);      %噪声子空间

%******************************第三部分：求空间谱****************************% 
for pitch=0%for pitch=1:180
for azimuth= -90:90%=1:180
...
%Pmusic(pitch,azimuth)=abs(8./WW);                         %空间谱
 Pmusic(1,azimuth+91)=abs(4./WW);  %Pmusic(1,azimuth)=abs(8./WW);Pmusic(1,azimuth+91)=abs(8./WW);
% Pmusic(1,pitch)=abs(8./WW);  
end %for azimuth=1:180
end %for pitch=0%pitch=1:182
